...
1. Primeiro iteramos em `getFieldErrors()` então coletamos as mensagens de erro de `getDefaultMessage()` retornando uma lista com elas:
<CodeHeader title="GlobalExceptionHandler.java" />
```java 
List<String> errors = ex.getBindingResult()
    .getFieldErrors()
...
```
As únicas diferenças do método do erro anterior é que trocamos a classe do `ExceptionHandler` para lidar com a nossa classe `NotFoundException`, e o jeito que coletamos a mensagem de erro. Dessa vez criamos uma lista que contém a mensagem de erro retornada com `List.of(ex.getMessage())`.
